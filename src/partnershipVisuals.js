 ...
 */
export const PartnershipVisuals = Class.create(AbstractNodeVisuals, {
  ...
  initialize: function($super, partnership, x, y) {
    ...
    this._junctionShape = editor.getPaper().circle(x,y, PedigreeEditorAttributes.partnershipRadius).attr(PedigreeEditorAttributes.partnershipNode);
  ...
  updateIDLabel: function() {
    ...
    this._idLabel = editor.getPaper().text(x, y-20, editor.DEBUG_MODE ? this.getNode().getID() : '').attr(PedigreeEditorAttributes.dragMeLabel).insertAfter(this._junctionShape.flatten());
  },
  ...
  getJunctionShape: function() {
     ...
     */      
  getBottomY: function() {
    return this._absoluteY + PedigreeEditorAttributes.partnershipRadius + PedigreeEditorAttributes.childlessLength;
  },    
  ...
  updatePartnerConnections: function() {
    ...
    var lineAttr = consangr ? PedigreeEditorAttributes.consangrPartnershipLines : PedigreeEditorAttributes.partnershipLines;
    ...
    var cornerRadius  = PedigreeEditorAttributes.curvedLinesCornerRadius;
    ...
    for (var p = 0; p < partnerPaths.length; p++) {
      ...
      var lastBend      = ((finalYTo == yTop) && (yTop < this.getY()) && finalSegmentInfo.attachmentPort == 1) ?
                                ...
                                ( finalSegmentInfo.numAttachPorts > 1 ?
                                   PedigreeEditorAttributes.radius * (1.8 + finalSegmentInfo.numAttachPorts*0.1 - finalSegmentInfo.attachmentPort*0.35) :
                                   PedigreeEditorAttributes.radius * 1.6
                                );
      ...
      if (lostContact) {
        ...
        var lineSize = PedigreeEditorAttributes.notInContactLineSize;
        editor.getPaper().path('M ' + (xCross) + ' ' + (this.getY() - lineSize) + ' L ' + (xCross) + ' ' + (this.getY() + lineSize)).attr(PedigreeEditorAttributes.noContactLines).toBack();
      }
    }
  ...
  updateChildhubConnection: function() {
    ...
    var twinCommonVerticalPieceLength = PedigreeEditorAttributes.twinCommonVerticalLength;        
    ...
    for ( var j = 0; j < children.length; j++ ) {
      ...
      if (twinGroupId != currentTwinGroup) {
        ...
        editor.getView().drawLineWithCrossings( id, currentTwinGroupCenterX, childlineY, currentTwinGroupCenterX, childlineY+twinCommonVerticalPieceLength, PedigreeEditorAttributes.partnershipLines);
        ...
        if (currentIsMonozygothic) {
          var twinlineY   = childlineY+PedigreeEditorAttributes.twinMonozygothicLineShiftY;
          ...
          editor.getView().drawLineWithCrossings( id, xIntercept1, twinlineY, xIntercept2, twinlineY, PedigreeEditorAttributes.partnershipLines); 
        }
      }
      ...
      if (!currentIsMonozygothic || childX == positionL || childX == positionR ) { 
        editor.getView().drawLineWithCrossings( id, topLineX, topLineY, childX, childY, PedigreeEditorAttributes.partnershipLines);
      }
      else {                
        ...
        editor.getView().drawLineWithCrossings( id, xIntercept, twinlineY, childX, childY, PedigreeEditorAttributes.partnershipLines);
      }
      ...
      if (lostContact) {
        if (twinGroupId == null) {
          var lineSize = PedigreeEditorAttributes.notInContactLineSize;
          editor.getPaper().path('M ' + (topLineX - lineSize) + ' ' + (topLineY + 20) + ' L ' + (topLineX + lineSize) + ' ' + (topLineY + 20)).attr(PedigreeEditorAttributes.noContactLines).toBack();
        } else {
        }
      }
    }

    editor.getView().drawLineWithCrossings( id, leftmostX, childlineY, rightmostX, childlineY, PedigreeEditorAttributes.partnershipLines);        
    editor.getView().drawLineWithCrossings( id, this.getX(), this.getY(), this.getX(), childlineY, PedigreeEditorAttributes.partnershipLines);
        
        //draw small non-functional childhub junction orb
    if (numPregnancies > 1)
      editor.getPaper().circle(this.getX(), childlineY, PedigreeEditorAttributes.partnershipRadius/2).attr({fill: '#666666', stroke: '#888888', 'stroke-width':1, 'opacity': 1});
