...
export const AbstractPersonVisuals = Class.create(AbstractNodeVisuals, {

    initialize: function($super, node, x, y) {
        $super(node, x, y);

        this._radius = PedigreeEditorAttributes.radius;
        this._width  = PedigreeEditorAttributes.radius * 4;
    ...
    updateIDLabel: function() {
        ...
        this._idLabel = editor.getPaper().text(x, y, this.getNode().getID()).attr(PedigreeEditorAttributes.dragMeLabel).toFront();
    ...
    updateNumberLabel: function() {
        ...
        if (this.getNode().getPedNumber() != "") {
            var x = this.getX();
            var y = this.getY();
            this._numberLabel = editor.getPaper().text(x, y, this.getNode().getPedNumber()).attr(PedigreeEditorAttributes.pedNumberLabel).toFront();
    ...
    setPos: function($super, x, y, animate, callback) {
        ...
        if(animate) {
            ...
            this.getAllGraphics().animate(
                {"transform": "t " + moveX + "," + moveY + "..."}, 900, "linear", me._callback
            ); //easeInOut
     ...
     */    
    getBottomY: function() {
        return this._absoluteY + this._radius + PedigreeEditorAttributes.childlessLength;
    },
    ...
    drawAdoptedShape: function() {
        ...
        var r = PedigreeEditorAttributes.radius,
            x1 = this.getX() - ((0.8) * r),
            x2 = this.getX() + ((0.8) * r),
            y = this.getY() - ((1.3) * r) + 1,
    ...
    setHighlightBox: function() {
        ...
        this._highlightBox = editor.getPaper().rect(this.getX()-radius, this.getY()-radius,
                                                    radius*2, radius*2, 5).attr(PedigreeEditorAttributes.boxOnHover);
