...
const getDataAndRender = (patientId, type) => {
  ...
  promise
    ...
    .catch(err => {
      if (err.status === 403) {
        jquery("body").append(
          '<p>please <a href="http://localhost:8000">log in</a></p>'
        );
      }
      console.trace(err);
    });
...
const createInput = () => {
  jquery("body").prepend(
    '<span>Patient ID: </span><input type="number" name="patientId" id="patientId" value="525"></input><button id="go">go</button> <a href="" id="525">525</a>, <a href="" id="9971">9971</a>, <a href="" id="4247">4247</a>'
  );
  jquery("body").prepend(
    '<span>Patient SNV ID: </span><input type="number" name="patientSnvId" id="patientSnvId" value="2919479"></input>'
  );
  jquery("body").prepend(
    '<span>Transcript ID: </span><input type="number" name="transcriptId" id="transcriptId" value="26989"></input>'
  );
  jquery("body").prepend(
    '<span>Gene name: </span><input type="text" name="geneName" id="geneName" value="ATM"></input>'
  );
  jquery("body").prepend(
    '<select id="type"><option value="segregation">Segregation</option><option value="overview">Overview</option></select>'
  );
  jquery("#go").on("click", e => {
    e.preventDefault();
    const type = jquery("#type").val();
    const patientId = jquery("#patientId")
      .val()
      .strip();
