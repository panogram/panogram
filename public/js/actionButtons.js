export const XWiki = (function(d) {
    var c = d.actionButtons = d.actionButtons || {};
    c.EditActions = Class.create({
        initialize: function() {
            this.addListeners();
            this.addShortcuts();
            this.addValidators();
        ...
        addShortcuts: function() {
            var e = {
                action_cancel: "Alt+C",
                action_preview: "Alt+P",
                action_edit: "Alt+B",
                action_inline: "Alt+B",
                action_save: "Alt+S",
                action_propupdate: "Alt+S",
                action_saveandcontinue: "Alt+Shift+S"
            };
            for (var f in e) {
                var g = $$("input[name=" + f + "]");
                if (g.size() > 0) {
        ...
        addValidators: function() {
            ...
            for (var h = 0; h < e.length; h++) {
                var f = e[h];
                var g = new LiveValidation(f, {
                    validMessage: ""
                });
                g.add(Validate.Presence, {
        ...
        validateForm: function(g) {
            ...
            if (h && (false || false)) {
                while (h.value == "") {
                    var e = prompt("Enter a brief description of your changes", "");
                    if (e === null) {
                        return false;
                    }
                    h.value = e;
        ...
        onCancel: function(f) {
            f.stop();
            this.notify(f, "cancel");
            var e = f.element().form.action;
            if (typeof e != "string") {
                e = f.element().form.attributes.getNamedItem("action");
                if (e) {
                    e = e.nodeValue;
                } else {
                    e = window.self.location.href;
                }
            }
            var g = e.split("#", 2);
            var h = (g.length == 2) ? g[1] : "";
            e = g[0];
            if (e.indexOf("?") == -1) {
                e += "?";
            }
            d.EditLock && d.EditLock.setLocked(false);
            window.location = e + "&action_cancel=true" + h;
        },
        ...
        notify: function(e, f, g) {
            document.fire("xwiki:actions:" + f, Object.extend({
                originalEvent: e,
                form: e.element().form
            }, g || {}));
            if (e.stopped) {
    ...
    c.AjaxSaveAndContinue = Class.create({
        initialize: function() {
            this.createMessages();
            this.addListeners();
        },
        createMessages: function() {
            this.savingBox = new d.widgets.Notification("Saving...", "inprogress", {
                inactive: true
            });
            this.savedBox = new d.widgets.Notification("Saved", "done", {
                inactive: true
            });
            this.failedBox = new d.widgets.Notification("Failed to save the document. Reason: <span id=\"ajaxRequestFailureReason\"\/>", "error", {
                inactive: true
            });
        },
        addListeners: function() {
        ...
        onSave: function(e) {
            ...
            if (e.memo["continue"]) {
                ...
                this.failedBox.hide();
                this.savingBox.show();
                var f = new Hash(this.form.serialize({
                    hash: true,
                    submit: "action_saveandcontinue"
                }));
                f.set("minorEdit", "1");
        ...
        onFailure: function(e) {
            ...
            document.fire("xwiki:document:saveFailed", {
                response: e
            });
        }
    });
